...
> Previous Versions: MASTER_DESIGN_v0.1_backup.md, MASTER_DESIGN_v0.2_backup.md
...
└── Testing/                    ← ISteamNativeBackend, FakeSteamBackend
...
public sealed class SteamLifecycle : IDisposable
{
    ...
    // Must be called once per frame on the game/main thread
...
> **Build plumbing note**: The symbol approach is conceptually correct but requires airtight CI coverage to be trustworthy. If `MANIFOLD_PACK_SMALL` is absent or misconfigured on a given platform, the wrong `Pack` value is silently applied — the compiler produces no error. The primary safety net is the interop size validation tests (§6.11), which load the native DLL and compare `Marshal.SizeOf<T>()` against native `sizeof(T)` at runtime on every platform in the CI matrix. If the symbol is wrong, the size test fails loudly. The CI matrix must build and run these tests on all three target platforms (Windows x64, Linux x64, macOS), never only on the development machine.
...
> **Validation requirement**: The assumption that Godot copies `rBuffer` before `_GetPacket` returns must be verified against Godot engine source during Phase 2 implementation. If this contract does not hold, fall back to a persistent per-peer `byte[]` sized at `_GetMaxPacketSize()` that is reused each frame (no pool, no allocation after init).
